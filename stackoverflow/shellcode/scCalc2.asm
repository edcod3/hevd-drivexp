; nasm -f win32 scCalc2.asm -o scCalc2.obj 
; bash: for i in $(objdump -D scCalc2.obj | grep "^ " | cut -f2); do echo -n '\x'$i; done; echo
_start:
    ...
    mov ebx, fs:[0x124]     ; Edit: EBX = Address_of_KTHREAD
    mov ebx, [ebx+0x150]    ; Edit: EBX = Address_of_EPROCESS
    mov ebx, [ebx+0x17C]    ; Edit:  EBX = Address_of_PEB
...
functions:
    ...
    mov ebx, eax          ; Edit: EBX now has the address of WinExec
    ...
    push eax              ; lpcmdLine - push string address stack # 1st argument

    ;FIXME: ntdll!NtdllpAllocateStringRoutine access violation
